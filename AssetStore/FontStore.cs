...
namespace PvZ_Fusion_Translator__BepInEx_.AssetStore
{
	public static class FontStore
	{
		...
		internal static void Init()
		{
			...
			InitFallback();
			#else
			string defaultFontDir = Path.Combine("PvZ_Fusion_Translator", "[Custom Fonts]", "Vietnamese.ttf");

			TMP_FontAsset defaultFont = FontHandler.LoadTMPFont(defaultFontDir, true);
			fontAssetDict.Add("Vietnamese", defaultFont);
			#endif
		}
		...
		public static void AddFallback()
		{
			foreach (var lang in fontAssetDict.Keys)
			{
				...
				{
					fontAssetDict[lang].fallbackFontAssetTable = new Il2CppSystem.Collections.Generic.List<TMPro.TMP_FontAsset>();
				}
		...
		public static TMP_FontAsset LoadTMPFont(string language)
		{
			...
			return fontAssetDict.GetValueOrDefault("Vietnamese");
		}
		#else
		public static TMP_FontAsset LoadTMPFont()
		{
			TMP_FontAsset defaultFont = fontAssetDict["Vietnamese"];
			return defaultFont;
		}
		#endif
